...
def fetch_status(selected_user, df):
    ...
    num_midea_messages = df[df['message'] == '<Media omitted>\n'].shape[0]
    ...
    return num_messages, len(words), num_midea_messages, len(links)
...
def weekly_activity_time(selected_user, df):
    if selected_user != 'Overall':
        df = df[df['user'] == selected_user]
        
    return df['day_name'].value_counts()
    
def monthly_activity_time(selected_user, df):
    if selected_user != 'Overall':
        df = df[df['user'] == selected_user]
